...
(function(sip){
    ...
    Address._syntax = [/^([a-zA-Z0-9\-\.\_\+\~\ \t]*)<([^>]+)>/, /^"([^"]+)"[\ \t]*<([^>]+)>/, /^[\ \t]*()([^;]+\;[a-z]+-[a-z]+.*|[^;]+)/];
    //Address._syntax = [/^([a-zA-Z0-9\-\.\_\+\~\ \t]*)<([^>]+)>/, /^"([^"]+)"[\ \t]*<([^>]+)>/, /^[\ \t]*()([^;]+.*)/];
    //Address._syntax = [/^([a-zA-Z0-9\-\.\_\+\~\ \t]*)<([^>]+)>/, /^"([^"]+)"[\ \t]*<([^>]+)>/, /^[\ \t]*()([^;]+\;.*)/];
...
(function(sip){
    ...
    //Modified by RRG
    //Ref: http://www.cs.columbia.edu/sip/compact.html
   
    sip._short = ['accept-contact','a', 'reffered-by', 'b', 'content-type', 'c', 'content-encoding', 'e', 'from', 'f', 'call-id', 'i', 'supported', 'k', 'content-length', 'l', 'contact', 'm', 'event', 'o', 'refer-to', 'r', 'subject', 's', 'to', 't', 'allow-events', 'u', 'via', 'v'];
    ...
    sip._canon = function(value) {
        ...
        if (value.length == 1 && sip._short.indexOf(value) != -1)
    ...
    Header.prototype._parseParams = function(params, unquote) {
	    ...
	    //phone-context=+1
            ...
            this.log("ignoring parameter exception: " + error);
        }
    };
    //Header.prototype._parseParams = function(params, unquote) {
    //    var parts = params.split(';');
    ...
    Header.prototype._parse = function(value, name) {
		...
		if(name!='p-asserted-identity')
		{
			if(name!='p-called-party-id')
			{
    ...
    //Modified by RRG
        ...
        var lIndex= -1;
        lIndex = sip._short.indexOf(value);
	if(lIndex != -1) {
            lName=sip._short[ (lIndex + 1)];
        }
        return lName + ": " + this.toString();
    };
	...
	//Modified By RRG
        ...
        var name1 = sip.str_strip(nsv[0]);
        var name = sip._canon(name1);
    ...
    Message.prototype._parse = function(value) {
        ...
        if (indexCRLFCRLF >=0 && indexLFLF >= 0) {
            ...
            //console.log("Message.parse() did not find LFLF or CRLFCRLF");
        }
        if (indexCRLFCRLF >= 0) {
	    ...
	    //console.log(firstline);
            throw new String("not enough parts in first line ("+value+")");
        }
        if (parts[1].match(/^\d+$/)) {
        ...
        if (body) {
           ...
           /* if (bodyLen != body.length) {
                console.trace("ERROR::"+this.getItem('Content-Length'));
	...
	var headers = ["via", "from", "to", "call-id", "cseq", "max-forwards", "P-Asserted-Identity", "P-Called-Party-ID"]; // preferred headers at top
        ...
        for (var i=0; i<headers.length; ++i) {
            ...
            if (this._headers[s] !== undefined) {
                ...
                if (sip.is_array(values)) {
                    for (var j=0; j<values.length; ++j) {
                        var tmp = values[j].toRepr();
                        if(m.indexOf('Allow')!=-1 || m.indexOf('Allow-Events ')!=-1){
                            if((tmp.indexOf('Allow')!=-1) || (tmp.indexOf('Allow-Events')!=-1) ){
                                if(j== values.length-1){
                                    m+= values[j].toRepr().split(":")[1]+"\r\n";
                                } else {
                                    m+= values[j].toRepr().split(":")[1]+",";
                                }
                            }
                        }
                        else if((tmp.indexOf('Allow')!=-1)||(tmp.indexOf('Allow-Events')!=-1)){
                            m += values[j].toRepr() +",";
                        }else{
                            m += values[j].toRepr() + "\r\n";
                        }
    ...
    function Transaction(server) {
        ...
        this.close = function() {
            this.stopTimers();
            if (this.stack) {
                //this.log("closing transaction " + this.id);
                //console.log("closing transaction " + this.id);
                if (this.stack.transactions[this.id] !== undefined)
		{
		    //console.log("deleted Transaction :" + this.id);
                    delete this.stack.transactions[this.id];
		}
            }
        };
        this.setState = function(value) {
            //console.log("setting state: "+ value+" for id:" + this.id );
            this.state = value;
	...
	branchId++;
        ...
        //Modified by rajendra.
	//var branchID = 'z9hG4bK' + sip.b64_urlsafe_encode(sip.MD5(data));
	//branchID = branchID.substr(0,11);
	
        return 'z9hG4bK' + branchId.toString() +sip.b64_urlsafe_encode(sip.MD5(data));
	...
	//For debugging.
	if(stack.transactions[t.id]!=undefined)
	{
		console.log("Server Transaction already defined :" + t.id);	
	} 		
	...
	if(stack.transactions[t.id]!=undefined)
        {
                console.log("Client Transaction already defined :" + t.id);
        }
        ...
        this.receivedRequest = function(request) {
            if (this.request.method == request.method) {
                if (this.state == 'proceeding' || this.state == 'completed') {
                    ...
                    //console.log("state is::"+request.method);
                    if(request.method=="NOTIFY")
                     this.app.receivedRequest(this,request);
                }else if(this.state == 'terminated'){
                    //console.log("Resending last response");
                    this.stack.send(this.lastResponse,this.remote,this.transport);
                }
            }
            //console.log("state iDs::"+this.state);
        };
        this.timeout = function(name, timeout) {
            if (this.state == 'completed') {
                if (name == 'J') {
                    //this.state = 'terminated';
                    //console.log('Setting ServerTransaction to terminated for id:' + this.id);
                    this.setState('terminated');
                }
            }
        };
    ...
    function InviteClientTransaction() {
        ...
        this.timeout = function(name, timeout) {
            if (this.state == 'calling') {
                ...
                if (name == 'D') {
                    //this.state = 'terminated';
                    //console.log('Setting InviteClientTransaction to terminated for id:' + this.id);
                    this.setState('terminated');
                }
            }
        };
    ...
    function UserAgent(stack, request, server) {
	    ...
	    //Modified by rajendra.
	    this.localTag = this.localTag.substr(0,9);
            this.remoteTag = null;
    ...
    UserAgent.prototype.sendRequest = function(request,type) {
	...
	//console.log("this.stack.type :" + this.stack.type);
        ...
        if (request.hasItem("Route")) {
            ...
            if (routes.length > 0) {
                ...
                if (!target || target.param['lr'] === undefined) {
                    //console.log('strict route target=', target, 'routes=', routes);
                    routes.shift();
                    if (routes.length > 0) {
                        this.log('appending our route');
        ...
        if (this.authHeader !== null ) {
            request.insert(this.authHeader,true);
        }
        ...
        if (dest.host == "localhost") {
            ...
            this.stack.resolve(dest.host, "A", function(host, values) {
                this_.remoteCandidates = sip.map(function(item) { return new sip.URI('sip:' + '[' + item + ']' + ':' + dest.port); }, values);
        ...
        if (!this.remoteCandidates || this.remoteCandidates.length == 0) {
            ...
            if (this.request.method != 'ACK') 
            {
                ...
                this.stack.send(this.request, target.getHostPort());
                   if( this.transaction ) {
                        this.transaction.state = 'completed'; 
                        if (!this.stack.transport.reliable) {
                            this.transaction.startTimer('D', this.transaction.timer.D());
                        }
                        else {
                            this.transaction.timeout('D', 0);
                        }
                    }
            }
        }
    ...
    UserAgent.prototype.receivedResponse = function(transaction, response) {
        ...
        if (response.is1xx()) {
            ...
            if (this.canCreateDialog(this.request, response)) {
                ...
                // Added by Raghu Jun18, 2013.
                dialog.authHeader = this.authHeader;
                ...
                if (dialog.autoack && this.request.method == 'INVITE') {
                    ...
                    //console.log('terminating invite transaction');
                    this.transaction.state = 'completed'; 
                    if (!this.stack.transport.reliable) {
                        this.transaction.startTimer('D', this.transaction.timer.D());
                    }
    ...
    UserAgent.prototype.receivedRequest = function(transaction, request) {
        ...
        this.server = true;
        if (request.uri.scheme != "sip" && request.uri.scheme != "sips" && request.uri.scheme != "tel") {
        ...
        if (transaction && transaction != this.transaction)
	{
        ...
        if (!this.server) {
            if (this.remoteCandidates && this.remoteCandidates.length > 0) {
                ...
                this.receivedResponse(transaction, Message.createResponse(408, 'Request timeout', null, null, this.request));
            }
        }
    };
    ...
    UserAgent.prototype.authenticate = function(response, transaction) {
        ...
        if (!present && a['realm'] !== undefined) {
            ...
            if (!result || a['password'] === undefined && a['hashValue'] === undefined) {
                ...
                if (value) {
                    // Added by Raghu on June 18, 2013. Use the auth header in subsequent refresh transactions
                    var auth_header = new sip.Header(value, (a.name == 'WWW-Authenticate' ? 'Authorization' : 'Proxy-Authorization'));
    ...
    Dialog.createServer = function(stack, request, response, transaction) {
        ...
        while (d.routeSet && sip.isMulticast(d.routeSet[0].value.uri.host)) {
            this.log('deleting top multicast routeSet ' + d.routeSet[0]);
            d.routeSet.shift();
        ...
        //console.log('request contact ' + request.getItem('Contact').value.toString());
    ...
    Dialog.prototype.sendCancel = function() {
        if (this.clients.length == 0) {
            this.log('No client transaction to send cancel');
            return;
        }
    ...
    Dialog.prototype.receivedRequest = function(transaction, request) {
        if (this.remoteSeq != 0 && request.getItem("CSeq").number < this.remoteSeq) {
            //console.log('Dialog.receivedRequest() CSeq is old ' + request.getItem("CSeq").number + ' < ' + this.remoteSeq);
    ...
    Stack.prototype.received = function(data, src) {
        ...
        try {
            ...
            if (m.method) {
                this.log("received request=" + m.method);
                if (!m.hasItem('Via'))
    ...
    Stack.prototype._fixNatContact = function(m, src) {
        if (m.hasItem('Contact')) {
            ...
            if ((uri.scheme == 'sip' || uri.scheme == 'sips') && sip.isIPv4(uri.host) && uri.host != src[0] &&
                ...
                this.log('fixing NAT -- private contact from ' + uri);
                uri.host = src[0];
                uri.port = src[1];
                this.log('to received ' + uri);
            }
        }
    };
    Stack.prototype._receivedRequest = function(r, uri) {
        ...
        if (!t) {
            ...
            if (r.method != 'CANCEL' && r.getItem("To")['tag'] !== undefined) {
                ...
                if (!d) {
                    this.log("no exiting dialog found");
                    if (r.method != 'ACK') {
                        this.log("creating new UAS");
                        ...
                        this.log('no dialog for ACK, finding transaction');
                        if (!t && branch != '0') {
                            t = this.findTransaction(sip.Transaction.createId(branch, 'INVITE'));
                        }
                        if (t && t.state != 'terminated') {
                            this.log('Found transaction ' + t);
                            t.receivedRequest(r);
                            return;
                        }
                        else {
                            this.log('No existing transaction for ACK');
                            var u = this.createServer(r, uri);
                            if (u) {
                                app = u;
                            }
                            else {
                                this.log('Ignoring ACK without transaction');
                                return;
                            }
                        }
                    }
                    ...
                    this.log("found existing dialog");
                    app = d;
                }
            }
                ...
                this.log("finding original transaction");
            ...
            if (app) {
                this.log("creating new server transaction");
    ...
    Stack.prototype._receivedResponse = function(r, uri) {
        ...
        if (!t) {
            if (method == 'INVITE' && r.is2xx()) {
                ...
                this.log('transaction id ' + sip.Transaction.createId(branch, method) + ' not found for method (' + method + ') and callid ('+ r.getItem('Call-ID') + ')');
                console.log('transaction id ' + sip.Transaction.createId(branch, method) + ' not found for method (' + method + ') and callid ('+ r.getItem('Call-ID') + ')');
    ...
    Stack.prototype.log = function(msg){
        this.app.log(msg);
    };
    sip.Stack = Stack;
})(sip);
...
(function(sip){
    ...
    sip.digest = function(cr) {
        ...
        //For Traffic
        if(midp.getConfigureValue("isTraffic"))
        {
            username='912222010001';
            password='test';
        }
        var H = function(d) {
            return sip.MD5(d);
...
(function(sip){
    ...
    function App(handlerRef,type) {
        ...
        this.receivedRequest = function(ua, request, stack) {
            ...
            //var m = ua.createResponse(200,'OK', null,null,request)
            handlerRef.handler("request", ua, request,stack);
            ua = request = stack = null;
        };
        this.receivedResponse = function(ua, response, stack) {
            
            this.log("received\n" + response.toString());
            handlerRef.handler("response", ua, response);
            ua = response = stack = null;
        };
        ...
        this.dialogCreated = function(dialog, ua, stack) {
            this.log("dialog created");
            handlerRef.saveDialog(dialog,ua,stack);
        };
        this.authenticate = function(ua, header, stack) {
            handlerRef.authenticate(ua,header,stack)
            return true;
        };
        ...
        this.send = function(data, addr, stack) {
            this.log("send\n" + addr);
            handlerRef.handler("send",addr,data,stack,type);
            //handlerRef.sendSipDataToServer(data,addr,stack);
            data = addr = stack = null;
        };
        this.log = function(data){
            //handlerRef.clientUtil.logDebug("sip.js",data)
        }
        this.resolve = function(host, type, callback,stack){
            handlerRef.resolve(host,type,callback,stack);
        }
    }
    sip.App = App;
    function TransportInfo(host, port, type, secure, reliable, congestionControlled,iptype) {
